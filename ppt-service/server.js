...
app.post("/generate-ppt", async (req, res) => {
  try {
...

    {
      ...
      kpis.forEach(([label, value], i) => {
        ...
        slide.addShape(pptx.shapes.RECTANGLE, {
          ...
          w: 0.1,
          h: 1.45,
          fill: { color: C.CHART[i] },
          line: { color: C.CHART[i] },
        });
...

    {
      ...
      companies.forEach((c, i) => {
        ...
        slide.addText(safe(c.upload_cadence), {
          ...
          fontFace: FONT_B,
          fontSize: 13,
          color: cadenceColor(c.upload_cadence),
          margin: 0,
        });
...

    {
      ...
      const companyLabels = companies.map(c => c.channel_name);
      const avgViews = companies.map(c => Number(c.average_views) || 0);
      const avgLikes = companies.map(c => Number(c.average_likes) || 0);
      const avgComments = companies.map(c => Number(c.average_comments) || 0);

      const viewsMax = smartAxisMax(avgViews);
      const likesMax = smartAxisMax(avgLikes);
      const commentsMax = smartAxisMax(avgComments);
      ...
      const bottomY = 4.2;
      const gap = 0.22;
      const totalWidth = chartW * 3 + gap * 2;
      const startX = (13.33 - totalWidth) / 2;
      ...
      slide.addChart(
        ...
        [{ name: "Avg Views", labels: companyLabels, values: avgViews }],
        {
          ...
          catGridLine: { style: "none" },
          valGridLine: { color: C.BG4, style: "solid", pt: 0.5 },
          gapWidthPct: 35,
          valAxisMinVal: 0,
      ...
      slide.addText("AVERAGE COMMENTS", {
        x: startX + (chartW + gap) * 2,
        y: topY - 0.22,
        w: chartW,
        h: 0.2,
      ...
      const insightText = safe(data.engagement_analysis);

      card(slide, pptx, 0.6, bottomY, 12.1, 1.45, C.BG3);

      slide.addText("ENGAGEMENT INSIGHTS", {
        x: 0.8,
        y: bottomY + 0.18,
        w: 11.7,
        h: 0.22,
      ...
      companies.forEach((c, i) => {
        const bw = 12.5 / companies.length;
        const bx = 0.35 + i * bw;
        const accent = C.CHART[i % C.CHART.length];

        card(slide, pptx, bx, engRowY, bw - 0.15, 0.82, C.BG3);
...

    {
      ...
      const cardH =
        (availH - (visibleCompanies.length - 1) * cardGap) / visibleCompanies.length;
    ...
    const formattedDate = `${saveDate.getFullYear()}-${String(saveDate.getMonth() + 1).padStart(
      2,
      "0"
    )}-${String(saveDate.getDate()).padStart(2, "0")}`;

    const publicFileName = `Video Competitor Intelligence Report ${formattedDate}.pptx`;
